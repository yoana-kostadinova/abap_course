CLASS lhc_Travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS global_authorization FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR zi_rap_travel_yk
      RESULT result.
...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD global_authorization.
    " Delegate to existing get_authorizations method for consistency
    result = VALUE #( %create = if_abap_behv=>auth-allowed ).
  ENDMETHOD.

  METHOD calculateTotalPrice.
    MODIFY ENTITIES OF ZI_RAP_Travel_YK IN LOCAL MODE
        ENTITY travel
  ...
  METHOD CalculateTravelID.
    ...
    READ ENTITIES OF ZI_RAP_Travel_YK IN LOCAL MODE
      ENTITY Travel
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky        = travel-%tky
                        ...
                        %msg        = NEW zcm_rap_yk(
                                          ...
                                          textid   = zcm_rap_yk=>agency_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_YK(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_YK=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_YK(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_YK=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
      ...
      IF is_update_requested = abap_true.
        ...
        IF has_before_image = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky        = travel-%tky
                            %msg        = NEW zcm_rap_YK( severity = if_abap_behv_message=>severity-error
                                                            textid   = zcm_rap_YK=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
...
*                      %action-Edit         = COND #( WHEN update_granted = abap_true THEN if_abap_behv=>auth-allowed ELSE if_abap_behv=>auth-unauthorized )
  ...
  METHOD is_update_granted.
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZOSTATYK '
        ID 'ZOSTATYK ' FIELD overall_status
        ID 'ACTVT' FIELD '02'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZOSTATYK '
        ID 'ZOSTATYK ' DUMMY
